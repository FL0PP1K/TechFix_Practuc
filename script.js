...
const firebaseConfig = {
  ...
  databaseURL:
    "https://techfix-base-default-rtdb.europe-west1.firebasedatabase.app",
  ...
  db.ref("reviews").on("value", (snapshot) => {
    ...
    if (reviewsData && reviewsContainer) {
      ...
      reviewsArray.forEach((rev) => {
        // ДОДАНО: Показуємо відгук ТІЛЬКИ якщо він схвалений адміном!
  ...
  db.ref("orders/" + input)
    ...
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        // ВІКНО ВІДГУКУ: Якщо статус містить слово "готово" або тип "done"
        if (order.status.toLowerCase().includes("готово") || order.type === "done") {
          setTimeout(() => {
            const modal = document.getElementById("review-modal");
            if (modal) modal.style.display = "flex";
          }, 1000); // Вискочить через 1 секунду
        }
...
function submitModalReview() {
  ...
  // Відправляємо в базу зі статусом "не схвалено"
  db.ref("reviews").push({
    name: name,
    text: text,
    date: new Date().toLocaleDateString(),
    approved: false // ДОДАНО: Відгук іде на модерацію
  }).then(() => {
    alert("Дякуємо! Ваш відгук відправлено на перевірку модератору.");
    closeReviewModal();
